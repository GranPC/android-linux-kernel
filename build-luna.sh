...
# TODO: perf?
make CONFIG_BBRY=1 O=out CROSS_COMPILE=$CROSS_COMPILE ARCH=$ARCH luna-perf_defconfig
# use config extracted from device for now
cp config-from-luna out/.config
make CONFIG_BBRY=1 O=out CROSS_COMPILE=$CROSS_COMPILE ARCH=$ARCH -j12
